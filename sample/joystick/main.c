...
void subscription_callback(const void * msgin)
{
	...
	syslog(LOG_NOTICE, "[%ld] ", msg->header.stamp.nanosec);
	syslog(LOG_NOTICE, "frame_id[%s] ", msg->header.frame_id.data);
	syslog(LOG_NOTICE, "angular_velocity[x:%f, y:%f, z:%f] ", msg->angular_velocity.x, msg->angular_velocity.y, msg->angular_velocity.z);
	syslog(LOG_NOTICE, "linear_acceleration[x:%f, y:%f, z:%f]\n", msg->linear_acceleration.x, msg->linear_acceleration.y, msg->linear_acceleration.z);
}
...
void main_task(intptr_t exinf)
{
	...
	syslog(LOG_NOTICE, "micro_ros_utilities_create_message_memory\n");
	...
	while(1){
		...
		if(temp_rc != RCL_RET_OK) {
			syslog(LOG_NOTICE, "publisher err %ld\n", temp_rc);
		}
